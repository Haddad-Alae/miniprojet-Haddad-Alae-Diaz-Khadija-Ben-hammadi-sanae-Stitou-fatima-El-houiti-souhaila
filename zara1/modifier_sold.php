...
<?php
 ...
 if(isset($_POST['modifier'])){
    ...
    if(!empty( $libelle) && !empty($prix)   && !empty($categories)){
        ...
        $sqlState = $pdo->prepare('UPDATE sold
                                    SET libelle=? ,
                                     prix=? ,
                                     discount= ?,
                                     id_categorie=?
...
<form method="post">
    ...
    <input type="hidden" class="form-control"   name="id" value="<?php echo  $sold['id']?>">
        <label class="form-label">libelle</label>
        <input type="text" class="form-control"  name="libelle" value="<?php echo  $sold['libelle']?>">
    </div>
   
        
    <label class="form-label">prix</label>
        <input type="number" step="0.1"  class="form-control" id="inputName"  name="prix" min="0"   value="<?php  echo  $sold['prix']  ?>">


        <label class="form-label">Remise</label>
        <input type="range" class="form-control" id="inputName"  name="discount"  value="<?php  echo   $sold['discount']  ?>"   >
...
foreach($categories as $categories){
    if($sold['id_categorie']== $categories['id']){
