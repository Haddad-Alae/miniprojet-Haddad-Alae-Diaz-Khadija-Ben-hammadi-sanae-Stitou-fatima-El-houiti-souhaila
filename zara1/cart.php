...
<?php
    ...
    $idsolds= array_keys($cart);
    $idsolds =implode(',',$idsolds);
    $idsolds='('.$idsolds.')';
    
    $solds = $pdo->query("SELECT*FROM sold WHERE id IN $idsolds")->fetchAll(PDO::FETCH_ASSOC);
...
$produits = $pdo->query("SELECT*FROM produit WHERE id IN $idProduits")->fetchAll(PDO::FETCH_ASSOC);
    ...
    <ul class="list-group list-group-flush w-25">
        ...
        foreach($solds as $sold){
            $idsold=$sold['id'];
            $tsold=$sold['prix'] *$cart[$idsold];
            $total+= $tsold;
            ?>
            <tr>
            
                    <td><?php echo $sold['id']?></td>
                    <td><img  width="80px "src="upload/<?php echo $sold['image'] ?>"></td>
                    <td><?php echo $sold['libelle']?></td>
                    <td><?php include'countersold.php' ?></td>
                    <td><?php echo $sold['prix'] ?> DH</td>
                    <td><?php echo $sold['prix'] *$cart[$idsold] ?> DH</td>
