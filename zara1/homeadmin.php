...
$nomuser=$pdo->query('SELECT * FROM user');
while($user = $nomuser->fetch()){
  //?>
  	<tr>
  	<td>
  <p> <?=  $user['login']; ?>  </p>
...
<td>  <p> <?=  $user['date_creation']; ?>  </p></td></tr>
  <?php
}
?>
...
while($user = $nomuser->fetch()){
  ...
  <!--<h6><i style="color:red;" class="fa-regular fa-circle-user"></i> <?=  $user['login']; ?>  </h6>-->
